...
- Après un bon déploiement : `authRelease` doit être aligné avec le code déployé et **`signinGoogleFullPageNav`: `true`** doit apparaître dans `/api/health`.
- Déployer le **dernier commit** : `vercelGitCommitSha` dans `/api/health` doit correspondre à `main`. Il n’existe plus de route publique de diagnostic session (`/api/debug-auth` retirée pour la sécurité).
